...
import org.summit.keepcraft.command.*;
import org.summit.keepcraft.data.*;
import org.summit.keepcraft.data.models.LootBlock;
import org.summit.keepcraft.listener.*;
...
public class Keepcraft extends JavaPlugin 
{
    ...
    public double getTeamImbalanceAt(Location location)
    {
        Plot plot = ListenerHelper.getIntersectedPlot(location, new ArrayList<Plot>(DataCache.retrieveAll(Plot.class)));
        
        if(plot == null) 
        {
            return 1;
        }
        else if(plot.isTNTable())
        {
            plot.resetLastExplosion();
            return 1;
        }
        else
        {
        ...
        for(User user : DataCache.retrieveAll(User.class))
        {
            ...
            else if(plot.isFactionProtected(user.getFaction()))
            {
                // The plot under attack matches this user's plot, they are a defender
                defenderCount++;
            }
            else if(plot == user.getCurrentPlot() && !plot.isFactionProtected(user.getFaction()))
            {
                // They are in the plot and the plot is not theirs, they are an attacker
                attackerCount++;
            }
        }
        
        long lastExplosionSeconds = (System.currentTimeMillis() - plot.getLastExplosion()) / 1000;
        ...
        {
            plot.setLastExplosion();
        }
        
        if(attackerCount == 0) return 1; // how? but better safe than sorry
